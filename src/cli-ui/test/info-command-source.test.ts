...
assert.match(source, /case "auth_state":[\s\S]*!startupInfoCapturedRef\.current[\s\S]*commitItems\(\[createInfoBannerItem\(nextAuthState, RUNTIME_WORKING_DIRECTORY, sessionIdRef\.current, tokenUsageRef\.current\)\]\)/);
assert.match(source, /command\?\.name === "info"[\s\S]*setInput\(""\)[\s\S]*commitItems\(\[createInfoBannerItem\(authState, RUNTIME_WORKING_DIRECTORY, sessionIdRef\.current, tokenUsageRef\.current\)\]\)/);
