...
assert.match(source, /case "auth_state":[\s\S]*!startupInfoCapturedRef\.current[\s\S]*createInfoBannerItem\([\s\S]*nextAuthState,[\s\S]*RUNTIME_WORKING_DIRECTORY,[\s\S]*sessionIdRef\.current,[\s\S]*\)/);
