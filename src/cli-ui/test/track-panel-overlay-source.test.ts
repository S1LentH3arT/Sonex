...
assert.doesNotMatch(componentsSource, /const SPOTIFY_TRACK_ARTIST_WIDTH = 16/);
assert.doesNotMatch(componentsSource, /const SPOTIFY_TRACK_INDEX_WIDTH = 3/);
...
assert.doesNotMatch(componentsSource, /const isPlaylistTrackPanel = \(panel: NonNullable<TrackPanelState>\): boolean =>/);
assert.doesNotMatch(componentsSource, /const formatSpotifyTrackPanelIndex = \(track: TrackPanelTrack\): string =>/);
assert.doesNotMatch(componentsSource, /padStartDisplayWidth\(track\.index, SPOTIFY_TRACK_INDEX_WIDTH\)/);
assert.doesNotMatch(componentsSource, /track\.queued \? `✓\$\{index\}` : ` \$\{index\}`/);
assert.doesNotMatch(componentsSource, /padDisplayWidth\(track\.artist, SPOTIFY_TRACK_ARTIST_WIDTH\)/);
assert.match(componentsSource, /borderColor=\{isSpotifyThemePanel \? SPOTIFY_GREEN : BORDER_BLUE\}/);
assert.doesNotMatch(componentsSource, /const spotifyIndex = formatSpotifyTrackPanelIndex\(track\);/);
assert.match(componentsSource, /const spotifyLine = formatTrackPanelLine\(track, TRACK_PANEL_ROW_WIDTH\);/);
assert.match(componentsSource, /const rowBackgroundColor = selected \? \(isSpotifyThemePanel \? SPOTIFY_GREEN : "#4b2f3a"\) : undefined;/);
assert.match(componentsSource, /const rowFill = rowBackgroundColor \? " "\.repeat\(Math\.max\(0, TRACK_PANEL_ROW_WIDTH - stringWidth\(/);
